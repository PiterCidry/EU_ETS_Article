...
# Models for groups of countries index for specific installation
for (i in 1:length(group_of_countries)) {
  tryCatch(
    {
      print(paste0("(All) Outer loop: ", i, "/", length(group_of_countries)))
      # Data selection
      DataAll <- Data[which(Data$Compliance_Country %in% group_of_countries[[i]] & !is.na(Data$Emission)),] %>%
...
remove(i, j, k, l, m, n, o, p, group_of_countries)
...
colnames(effects) <- c("Group of countries", "Sector", "Model", "Year", "Effect")
effects <- effects %>% mutate(Year = as.numeric(Year), Effect = as.numeric(Effect))

write.xlsx(results, file = "Results/Results_Emission_groups_sectors_installations_years.xlsx", showNA = FALSE, row.names = FALSE)
write.xlsx(effects, file = "Results/Effects_Emission_groups_sectors_installations_years.xlsx", showNA = FALSE, row.names = FALSE)
